...
export const calcularReajuste = (valor, primeiroMes, primeiroAno, ultimoMes, ultimoAno) => {
    const ipcaFiltrado = colecaoInf.filter((item) => {
        const anoMes = item.ano * 12 + item.mes;
        const primeiroAno = primeiroAno * 12 + primeiroMes;
        const ultimoAno = ultimoAno * 12 + ultimoMes;
        return anoMes >= primeiroAno && anoMes <= ultimoMes;
    });
